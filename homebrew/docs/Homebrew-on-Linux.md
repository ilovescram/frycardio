...
The installation script installs Homebrew to `/home/linuxbrew/.linuxbrew` using _sudo_. Homebrew does not use _sudo_ after installation. Using `/home/linuxbrew/.linuxbrew` allows the use of most binary packages (bottles) which will not work when installing in e.g. your personal home directory.
...
Follow the _Next steps_ instructions to add Homebrew to your `PATH` and to your bash shell rcfile, either `~/.bashrc` for `bash` or `~/.zshrc` for `zsh`.
...
If you're using an older distribution of Linux, installing your first package will also install a recent version of _glibc_ and _gcc_. Use `brew doctor` to troubleshoot common issues.
